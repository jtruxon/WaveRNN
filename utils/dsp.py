...
def save_wav(x, path):
    soundfile.write(path, x.astype(np.float32), sr=hp.sample_rate)
...
def reconstruct_waveform(mel, n_iter=32):
    ...
    S = librosa.feature.inverse.mel_to_stft(
        amp_mel, power=1, sr=hp.sample_rate, n_fft=hp.n_fft, fmin=hp.fmin
    )
    wav = librosa.core.griffinlim(
        S, n_iter=n_iter, hop_length=hp.hop_length, win_length=hp.win_length
    )
    return wav
